...
window.onload = function() {
	...
	var injectedOnMessage = function(e) {
		...
		if (textMsg.startsWith('tile:')) {
			...
			let tileposx=0, tileposy=0, tilewidth=0, tileheight=0;
			for (let i = 0; i < splitMsg.length; ++i) {
				let msg = splitMsg[i];
				if (msg.startsWith('tileposx'))
					tileposx = msg.split('=')[1];
				else if (msg.startsWith('tileposy'))
					tileposy = msg.split('=')[1];
				else if (msg.startsWith('tilewidth'))
					tilewidth = msg.split('=')[1];
				else if (msg.startsWith('tileheight'))
					tileheight = msg.split('=')[1];
			}
			let sendMsg = `tileprocessed tile=0:${tileposx}:${tileposy}:${tilewidth}:${tileheight}:0`;
